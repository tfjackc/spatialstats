...
```{r echo=TRUE}
rng <- quantile(crimesstlagg$count, probs = c(0,0.25,0.5,0.75,1)) 
iqr <- IQR(crimesstlagg$count)
out <- boxplot.stats(crimesstlagg$count)$out
out_ind <- which(crimesstlagg$count %in% c(out))
summary(crimesstlagg$count)
out
```
