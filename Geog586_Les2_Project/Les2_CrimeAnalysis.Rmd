...
```
**Figure 1.** In combined years 2013 and 2014 Arson is the most common crime followed by DUI, and homicide. This chart is informative, and accurate, but may be misleading to state that our findings conclude that arson is consistently more common that DUIs in St. Louis.

## Temporal patterns associated with each crime 

Now let's dive deeper into the data, and start to visualize information based on when it took place. Perhaps with understanding when the events took place, we can start to see if there a particular reason or event that is skewing our data to a misguided interpretation. I would hypothesis in this stage of the process that an event occurred in 2013 or 2014 that led to an extreme level of arson.
...
Now that have seen the population of our data summed into one chart that gives an interesting perspective that the month of august experienced and above average number of crimes. We know very little of what kind of events took place. Utilizing a facet wrapped plot based on crime type and year we can see when each category experienced peaks and troughs based on monthly intervals. 
